...
@section('title', 'Detail - '.$order->service->title.'')
@section('content')
                                    ...
                                    <td class="px-1 py-5 text-sm w-2/8">
                                        <div class="flex items-center text-sm">
                                            <div class="relative w-10 h-10 mr-3 rounded-full md:block">
                                                @if (isset($order->user_freelancer->detail_user->photo))
                                                    <img class="object-cover w-full h-full rounded-full" src="{{ Storage::url($order->user_freelancer->detail_user->photo) }}" alt="" loading="lazy" />
                                                @else
                            ...
                            <div class="m-auto text-center">
                                ...
                                @if (isset($order->file))
                                    <div class="relative mt-0 md:mt-6">
                                        <a href="{{ Storage::url($order->file ?? '') }}" onclick="return confirm('Are you sure want to download this file?')" class="px-4 py-2 mt-2 text-left text-gray-700 rounded-xl bg-serv-hr">
                                            Download File
                                        </a>
                                    </div>
                                @else
                                    <div class="relative mt-0 md:mt-6">
                                        <a href="JavaScript:void(0)" class="px-4 py-2 mt-2 text-left text-gray-700 rounded-xl bg-serv-hr">
                                            File not available
                                        </a>
                                    </div>
                                @endif
                        ...
                        <form action="#" method="POST">
                            <div class="">
                                ...
                                <div class="px-1 py-4 text-right">
                                    <a href="{{ route('member.request.index') }}" type="button" onclick="return confirm('Are you sure want to back?, any changes you make will not be saved.')" class="inline-flex justify-center px-4 py-2 mr-4 text-sm font-medium text-gray-700 bg-white border border-gray-600 rounded-lg shadow-sm hover:bg-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-300">
                                        Back
                                    </a>
                                    @if ($order->order_status_id == 2 && isset($order->file))
                                        <a href="{{ route('member.approve.request', $order->id) }}" type="button" onclick="return confirm('Are you sure want to approve this data?')" class="inline-flex justify-center px-4 py-2 text-sm font-medium text-white bg-green-600 border border-transparent rounded-lg shadow-sm hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">
                                            Approve
                                        </a>
                                    @else
                                        <a href="JavaScript:void(0)" type="button" class="inline-flex justify-center px-4 py-2 text-sm font-medium text-white bg-green-600 border border-transparent rounded-lg shadow-sm hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">
                                            Approve
                                        </a>
                                    @endif
                                </div>
