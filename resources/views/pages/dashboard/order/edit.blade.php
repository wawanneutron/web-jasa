                            ...
                            <tbody class="bg-white">
                                <tr class="mb-10 text-gray-700">
                                    ...
                                    <td class="w-2/6 px-1 py-5">
                                        <div class="flex items-center text-sm">
                                            <div class="relative w-10 h-10 mr-3 rounded-full md:block">
                                                @if (isset($order->service->thumbnail_service->first()->thumbnail))
                                                    <img class="object-cover w-full h-full rounded" src="{{ Storage::url($order->service->thumbnail_service->first()->thumbnail) }}" alt="" loading="lazy" />
                                                @else
                                                    <span class="object-cover w-full h-full rounded-full">
                                                        <svg class="w-full h-full text-gray-300" fill="currentColor" viewBox="0 0 24 24">
                                                            <path d="M24 20.993V24H0v-2.996A14.977 14.977 0 0112.004 15c4.904 0 9.26 2.354 11.996 5.993zM16.002 8.999a4 4 0 11-8 0 4 4 0 018 0z" />
                        ...
                        <form action="{{ route('member.order.update', $order->id) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="flex p-8 border border-gray-200 rounded-lg bg-serv-upload-bg h-128">
                                <div class="m-auto text-center">
                                    ...
                                    <div class="relative mt-0 md:mt-6">
                                        @if (isset($order->file))
                                            <a href="{{ Storage::url($order->file ?? '') }}" onclick="return confirm('Are you sure want to download this file?')" for="choose" type="button" class="px-4 py-2 mt-2 text-left text-gray-700 rounded-xl bg-serv-hr">
                                                Download File
                                            </a>
                                        @else
                                            <input type="file" name="file" accept=".zip" hidden id="choose">
                                            <label for="choose" type="button" class="px-4 py-2 mt-2 text-left text-gray-700 rounded-xl bg-serv-hr">
                                                Choose File
                                            </label>
                                        @endif
                                        @if ($errors->has('file'))
                            ...
                            <div class="">
                                <div class="p-1 mt-5">
                                    <div class="grid grid-cols-6 gap-6">
                                        <div class="col-span-6">
                                            ...
                                            <textarea placeholder="Enter your biography here.." type="text" name="note" id="note" autocomplete="note" class="block w-full py-3 mt-1 border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500 sm:text-sm" rows="4" required {{ isset($order->file) ? 'disabled' : '' }}>{{ $order->note ?? '' }}</textarea>
                                            @if ($errors->has('note'))
                                ...
                                <div class="px-1 py-4 text-right">
                                    <a href="{{ route('member.order.index') }}" type="button" onclick="return confirm('Are you sure want to back?, any changes you make will not be saved.')" class="inline-flex justify-center px-4 py-2 mr-4 text-sm font-medium text-gray-700 bg-white border border-gray-600 rounded-lg shadow-sm hover:bg-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-300">
                                        Back
                                    </a>
                                    @if (isset($order->file) == false)
                                        <button type="submit" onclick="return confirm('Are you sure want to approve this data?')" class="inline-flex justify-center px-4 py-2 text-sm font-medium text-white bg-green-600 border border-transparent rounded-lg shadow-sm hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">
                                            Approve
                                        </a>
                                    @else
                                        <button type="button" class="inline-flex justify-center px-4 py-2 text-sm font-medium text-white bg-green-600 border border-transparent rounded-lg shadow-sm hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">
                                            Approve
                                        </a>
                                    @endif
                                </div>
