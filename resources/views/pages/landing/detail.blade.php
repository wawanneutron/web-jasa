                    ...
                    <div x-data="{ tab: window.location.hash ? window.location.hash.substring(1) : 'description' }" id="tab_wrapper">
                        ...
                        <div x-show.transition.duration.500ms="tab === 'seller'" class="leading-8 text-md">
                                ...
                                <div class="flex items-center col-span-12 p-2 lg:col-span-6">
                                    <div class="flex items-center space-x-4">
                                        @if ($service->user->detail_user->photo != null)
                                            <img alt="My profile" class="w-20 h-20 rounded-full "
                            ...
                            <ul class="mb-8 list-check">
                                @foreach ($advantage_user as $advantage_user_item)
            ...
            <aside class="p-4 lg:col-span-4 md:col-span-12 md:pt-0">
                <div class="mb-4 border rounded-lg border-serv-testimonial-border">
                    ...
                    <div class="flex items-start px-4 pt-6">
                        @if ($service->user->detail_user->photo != null)
                            <img alt="My profile" class="object-cover w-16 h-16 mr-4 rounded-full"
                                src="{{ Storage::url($service->user->detail_user->photo) }}" alt="random user" />
                        @else
                            <span class="object-cover w-16 h-16 mr-4 rounded-full">
                                <svg class="w-full h-full text-gray-300" fill="currentColor" viewBox="0 0 24 24">
                                    <path d="M24 20.993V24H0v-2.996A14.977 14.977 0 0112.004 15c4.904 0 9.26 2.354 11.996 5.993zM16.002 8.999a4 4 0 11-8 0 4 4 0 018 0z" />
                                </svg>
                            </span>
                    ...
                    <div class="px-4 pt-4 pb-2 features-list">
                        <ul class="mb-4 text-sm list-check">
                            @foreach ($advantage_service as $item_advantage_service)
                                <li class="pl-10 my-4">{{ $item_advantage_service->advantage }}</li>
                            @endforeach
                        </ul>
                    </div>
                    <div class="px-4">
                    ...
                    @guest
                        <div class="px-4 pb-4 booking">
                            <a href="JavaScript:void(0)" onclick="toggleModal('loginModal')" class="block px-12 py-4 my-2 text-lg font-semibold text-center text-white bg-serv-button rounded-xl">
                                Booking Now
                            </a>
                        </div>
                    @endguest
